...
const useSensorData = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setData(prev => ({
        temperature: Math.max(18, Math.min(35, prev.temperature + (Math.random() - 0.5) * 1.5)),
        ...
        lightLevel: Math.max(200, Math.min(1200, prev.lightLevel + (Math.random() - 0.5) * 50)),
        plantHealth: Math.max(60, Math.min(100, prev.plantHealth + (Math.random() - 0.5) * 1)),
        waterLevel: Math.max(10, Math.min(100, prev.waterLevel + (Math.random() - 0.5) * 2)),
        energyUsage: Math.max(1, Math.min(5, prev.energyUsage + (Math.random() - 0.5) * 0.3))
      }));
    }, 2000);
