...
// === Profile Dropdown Component (Replaces AuthButtons) ===
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className={`flex items-center space-x-2 px-3 py-2 rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 ${
          isScrolled
            ? 'bg-white/10 hover:bg-white/20 text-white focus:ring-white/30'
            : 'bg-gray-100 hover:bg-gray-200 text-gray-800 focus:ring-green-300'
        }`}
      ...
      >
        <div
          className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${
            isScrolled ? 'border-green-400' : 'border-green-500'
          }`}
        >
          <User className={`w-5 h-5 ${isScrolled ? 'text-green-400' : 'text-green-600'}`} />
        </div>
      ...
      {isDropdownOpen && (
        <div
          className={`absolute right-0 mt-2 w-56 rounded-xl shadow-2xl overflow-hidden z-50 border ${
            isScrolled
        ...
        >
          ...
          >
            <p className="font-medium">{state.displayName || "User"}</p>
            <p
          ...
          <div className="py-1">
            ...
            <hr className={isScrolled ? 'border-white/10' : 'border-gray-200'} />
            <button
              onClick={handleSignOut}
              className={`w-full text-left px-4 py-3 text-sm flex items-center space-x-2 hover:bg-opacity-80 transition-colors ${
                isScrolled
...
const GlobalNavbar = () => {
  ...
  // ✅ Updated navItems with paths for navigation
  const navItems = [
    {
      name: 'Solutions',
      hasDropdown: true,
    ...
    },
    {
      ...
      hasDropdown: true,
      items: [
        { name: 'Innovators', icon: User, desc: 'Connect with pioneers' },
    ...
    { name: 'Impact', path: '/impact', hasDropdown: false },
    { name: 'Help', path: '/diagnosis', hasDropdown: false },
  ];
