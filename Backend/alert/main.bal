...
configurable string DB_PORT = "3306";
configurable string DB_NAME = "alert_db1";
configurable string DB_USER = "root";
configurable string DB_PASSWORD = "J3007426@Januda";

string jdbcUrl = string `jdbc:mysql://${DB_HOST}:${DB_PORT}/${DB_NAME}`;
...
// WebSocket listener and client management
...
// WebSocket service class for managing user connections
...
// Function to broadcast alert messages to all connected clients
...
// HTTP API service with CORS configuration
@http:ServiceConfig {
    cors: {
...
service /api on new http:Listener(8070) {
    ...
    function init() returns error? {
        self.dbClient = check new jdbc:Client(jdbcUrl, DB_USER, DB_PASSWORD);
        check initDatabase(self.dbClient);
    }

    // Get all alerts or filter by category
